...
data_loc_avec = data_loc[!is.na(data_loc$age_coemprunteur),]
...
data_loc_sans = data_loc[is.na(data_loc$age_coemprunteur),]
donnees_manquantes_l2 <- colSums(is.na(data_loc_sans)) / nrow(data_loc_sans) * 100
lapply(data_loc_sans, unique)
data_loc_sans = subset(data_loc_sans, select = -c(age_coemprunteur, situation_familliale_coemprunteur, type_contrat_coemprunteur))
...
data_prop_avec = data_prop[!is.na(data_prop$age_coemprunteur),]
donnees_manquantes_p1 <- colSums(is.na(data_prop_avec)) / nrow(data_prop_avec) * 100
lapply(data_prop_avec, unique)

data_prop_sans = data_prop[is.na(data_prop$age_coemprunteur),]
donnees_manquantes_p2 <- colSums(is.na(data_prop_sans)) / nrow(data_prop_sans) * 100
lapply(data_prop_sans, unique)
data_prop_sans = subset(data_prop_sans, select = -c(age_coemprunteur, situation_familliale_coemprunteur, type_contrat_coemprunteur))
